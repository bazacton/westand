...
if (!isset($cs_node->cs_event_view) || empty($cs_node->cs_event_view)) {
    $cs_node->cs_event_view = 'eventlisting';
}
$meta_compare = '';
$filter_category = '';
...
if (isset($cs_node->event_element_size) && $cs_node->event_element_size == '33') {
    ?>
    <div class="element_size_<?php echo cs_allow_special_char($cs_node->event_element_size); ?>">
        <div class="widget widget-latest-event">
            <?php if ($cs_node->cs_event_title <> '') { ?>
                <header class="cs-heading-title">
                    <h2 class="cs-section-title"><?php echo cs_allow_special_char($cs_node->cs_event_title); ?></h2>
                </header>
            <?php } ?> 
            ...
            while ($custom_query->have_posts()): $custom_query->the_post();
                ...
                <article <?php post_class($post_class); ?>>
                    <?php if ($event_count == 1) { ?>
                        <figure>
                            <?php if ($image_url <> '') { ?>
                                <img src="<?php echo cs_allow_special_char($image_url); ?>" alt="image_url">
                            <?php } ?>       
                            <figcaption>
            ...
            <?php if ($cs_node->cs_event_view_all_link <> "") { ?>
                <div class="fullwidth">
                    <a href="<?php echo cs_allow_special_char($cs_node->cs_event_view_all_link); ?>" class="btnviewall"><i class="fa fa-list-ul"></i><?php
                        if ($cs_theme_option['trans_switcher'] == "on") {
                            _e('View all events', 'westand');
                        } else {
                            echo cs_allow_special_char($cs_theme_option['trans_sortby']);
                        }
                        ?></a>
                </div>
            <?php } ?>
...
<?php } else { ?>
    <div class="element_size_<?php echo cs_allow_special_char($cs_node->event_element_size); ?>">
        <?php if ($cs_node->cs_event_title <> '' || $cs_node->cs_event_filterables == "Yes") { ?>
            <header class="cs-heading-title">
                ...
                if ($cs_node->cs_event_filterables == "Yes") {
                    ...
                    ?>
                    <div class="sortby">
                        <a class="btn sort-link" href="#"><i class="fa fa-list-ul"></i><?php
                            if ($cs_theme_option['trans_switcher'] == "on") {
                                _e('Events Sort By', 'westand');
                            } else {
                                echo cs_allow_special_char($cs_theme_option['trans_sortby']);
                            }
                            ?></a>
                        <ul>
                            ...
                            if (isset($cs_node->cs_event_category) && ($cs_node->cs_event_category <> "" && $cs_node->cs_event_category <> "0") && isset($row_cat->term_id)) {
                                ...
                                ?>
                                <li class="<?php
                                if (($cs_node->cs_event_category == $filter_category)) {
                                    echo 'bgcolr';
                                }
                                ?>">
                                    <a href="?<?php echo cs_allow_special_char($qrystr . "&filter_category=" . $row_cat->slug); ?>"><?php _e("All", 'westand'); ?></a>
                                </li>
                                <?php
                            } else {
                            ...
                            foreach ($categories as $category) {
                                ...
                                ?>><a href="?<?php echo cs_allow_special_char($qrystr . "&filter_category=" . $category->slug); ?>"><?php echo cs_allow_special_char($category->cat_name); ?></a>
                                </li>
                            <?php } ?>
                        </ul>
                    </div>
        ...
        <?php } ?>
        <div class="<?php echo cs_allow_special_char($cs_event_view); ?>">
            ...
            if ($custom_query->have_posts() <> "") {
                ...
                if ($cs_node->cs_event_view == 'eventlisting' || $cs_node->cs_event_view == 'event-gridview') {
                    ...
                    while ($custom_query->have_posts()): $custom_query->the_post();
                        ...
                        if( isset( $cs_event_meta->event_address) && !empty($cs_event_meta->event_address )){
                            $event_address = (array) $cs_event_meta->event_address;
                            $event_address = isset($event_address[0])? $event_address[0] : '';
                        }
                                    ...
                                    <?php if ($cs_event_meta->event_address <> "" && $event_loc_lat <> "" && $event_loc_long <> "" && $event_loc_zoom <> '') { ?>
                                        <script type="text/javascript">
                                            jQuery(document).ready(function () {
                                                event_map("<?php echo cs_allow_special_char($loc_address) . '<br>' . $loc_city . '<br>' . $loc_postcode . '<br>' . $loc_country ?>",<?php echo cs_allow_special_char($event_loc_lat); ?>, <?php echo cs_allow_special_char($event_loc_long) ?>,<?php echo cs_allow_special_char($event_loc_zoom) ?>,<?php echo cs_allow_special_char($post->ID); ?>);
                                            });
                                        </script>
                                        <div id="map_canvas<?php echo cs_allow_special_char($post->ID); ?>" class="event-map" style="height:<?php echo cs_allow_special_char($height); ?>px; width:<?php echo cs_allow_special_char($width); ?>px; float: left;"></div>
                                    <?php } ?>
                                    ...
                                    if ($cs_node->cs_event_time == "Yes") {
                                        ...
                                        <?php if ($cs_event_meta->event_all_day != "on") { ?>
                                            <li>
                                                ...
                                                <time datetime="2013-12-04"><?php echo cs_allow_special_char($event_start_time . ' ' . $to . ' ' . $event_end_time); ?></time>
                                            </li>
                                        <?php } else { ?>
                                            <li>
                                                <i class="fa fa-clock-o"></i>
                                ...
                                <div class="bottom-event">
                                    <?php if ($cs_event_meta->event_ticket_options <> "") { ?> 
                                        ...
                                        <?php if ($cs_event_meta->event_buy_now <> "") { ?> 
                                            <a class="buy-ticket btn" <?php if ($cs_event_meta->event_ticket_color <> '') { ?>style="background-color: <?php echo cs_allow_special_char($cs_event_meta->event_ticket_color); ?>" <?php } ?> href="<?php echo cs_allow_special_char($cs_event_meta->event_buy_now); ?>"><i class="fa fa-ticket"></i> <?php echo cs_allow_special_char($cs_event_meta->event_ticket_options); ?></a>
                                            <?php
                                        } else {
                                            ?> 
                                            <span class="buy-ticket btn" <?php if ($cs_event_meta->event_ticket_color <> '') { ?>style="background-color: <?php echo cs_allow_special_char($cs_event_meta->event_ticket_color); ?>" <?php } ?>><?php echo cs_allow_special_char($cs_event_meta->event_ticket_options); ?></span>
                                            <?php
                                        }
                                    }
                                    ...
                                    if (isset($cs_event_meta->event_like) && $cs_event_meta->event_like == "on") {
                                        ...
                                        if (isset($_COOKIE["cs_like_counter" . get_the_id()])) {
                                            ?>
                                            <a class="liks btn"> <i class="fa fa-thumbs-up"></i> <?php
                                                if ($cs_theme_option['trans_switcher'] == "on") {
                                                    _e('Likes', 'westand');
                                                } else {
                                                    echo cs_allow_special_char($cs_theme_option['trans_likes']);
                                                }
                                                ?><span><?php echo cs_allow_special_char($cs_like_counter); ?></span></a>
                                        <?php } else { ?>
                                            <a  class="likethis liks btn" href="javascript:cs_like_counter('<?php echo cs_allow_special_char($ajax_url); ?>','<?php echo get_template_directory_uri() ?>',<?php echo get_the_id() ?>)" id="like_this<?php echo get_the_id() ?>" > <i class="fa fa-heart-o"></i>  <?php
                                                if ($cs_theme_option['trans_switcher'] == "on") {
                                                    _e('Likes', 'westand');
                                                } else {
                                                    echo cs_allow_special_char($cs_theme_option['trans_likes']);
                                                }
                                                ?><span class="count-numbers like_counter<?php echo get_the_id() ?>"><?php echo cs_allow_special_char($cs_like_counter); ?></span></a>
                                            <a class="likes liks btn" id="you_liked<?php echo get_the_id() ?>" style="display:none;"><i class="fa fa-heart"></i> <?php
                                                if ($cs_theme_option['trans_switcher'] == "on") {
                                                    _e('Likes', 'westand');
                                                } else {
                                                    echo cs_allow_special_char($cs_theme_option['trans_likes']);
                ...
                <?php } elseif ($cs_node->cs_event_view == 'event-timeline') { ?>
                    ...
                    if ($cs_node->cs_event_featured_category <> '' && $cs_node->cs_event_featured_category <> '0') {
                        ...
                        while ($cs_featured_post->have_posts()) : $cs_featured_post->the_post();
                            ...
                            <script>
                                jQuery(document).ready(function ($) {
                                    cs_event_countdown('<?php echo cs_allow_special_char($year_event); ?>', '<?php echo cs_allow_special_char($month_event); ?>', '<?php echo cs_allow_special_char($date_event); ?>');
                                });
                            </script>
                        ...
                        <article>
                            ...
                            <div class="text">
                                <div class="left-timeline-sec">
                                    <h2  class="cs-post-title"><a href="<?php the_permalink(); ?>"><?php
                ...
                <?php } ?>
                                ...
                                <div class="right-timeline-sec">
                                    ...
                                    if (isset($_COOKIE["cs_like_counter" . get_the_id()])) {
                                        ?>
                                        <a class="btnlikes"> <i class="fa fa-thumbs-up"></i> <?php echo cs_allow_special_char($cs_like_counter); ?> <span></span></a>
                                    <?php } else {
                                        ?>
                                        <a  class="likethis btnlikes" href="javascript:cs_like_counter('<?php echo cs_allow_special_char($ajax_url) ?>','<?php echo get_template_directory_uri() ?>',<?php echo get_the_id() ?>)" id="like_this<?php echo get_the_id() ?>" > <i class="fa fa-heart-o"></i>  <span class="count-numbers like_counter<?php echo get_the_id() ?>"><?php echo cs_allow_special_char($cs_like_counter); ?></span></a>
                                        <a class="likes btnlikes" id="you_liked<?php echo get_the_id() ?>" style="display:none;"><i class="fa fa-heart"></i> <span class="count-numbers like_counter<?php echo get_the_id() ?>"><?php echo cs_allow_special_char($cs_like_counter); ?></span> </a>
                                        <div id="loading_div<?php echo get_the_id() ?>" style="display:none;"><i class="fa fa-spin"></i></div>
                ...
                <?php } ?> 
                    ...
                    <?php endwhile; ?>
                    <?php
                } elseif ($cs_node->cs_event_view == 'event-calendarview') {
                    cs_calender_enqueue_scripts();
                    $event_calendar = '';
                    if (isset($_GET['filter_event'])) {
                        $filter_event = $_GET['filter_event'];
                        echo cs_allow_special_char($filter_event);
                    }
                    while ($custom_query->have_posts()): $custom_query->the_post();
                        $cs_event_meta = get_post_meta($post->ID, "cs_event_meta", true);
                        $event_from_date = get_post_meta($post->ID, "cs_event_from_date", true);
