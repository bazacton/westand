...
<div class="element_size_<?php echo cs_allow_special_char($cs_node->cause_element_size); ?>">

    	<?php if ($cs_node->cause_title <> '') { ?>
			<header class="cs-heading-title">
            	<h2 class="cs-section-title"><?php echo cs_allow_special_char($cs_node->cause_title);?></h2>
			</header>
         <?php }?>
...
if($cs_theme_option['trans_switcher'] == "on"){ $cause_status_trans = __('Closed','westand');}else{ $cause_status_trans = $cs_theme_option['cause_status']; }
		 if( isset($cs_node->cause_type) && $cs_node->cause_type == "Upcoming Causes" ){
			...
			if($cs_tr <> ''){
				...
				if ($cs_xmlObject->cause_goal_amount > 0){
         ...
         <?php }?>
            <div class="text">
                ...
                <div class="progress-desc">
                    <span class="progress-box-left"> <?php if($cs_theme_option['trans_switcher'] == "on"){ $trans_featured = _e('Goal','westand');}else{ echo cs_allow_special_char($cs_theme_option['cause_goal']); }?> <?php echo cs_allow_special_char($cs_theme_option['paypal_currency_sign']);?><?php echo number_format((float)$cs_xmlObject->cause_goal_amount);?> </span>
                    <span class="progress-box-right"><?php if($cs_theme_option['trans_switcher'] == "on"){ _e('Raised','westand');}else{ echo cs_allow_special_char($cs_theme_option['cause_raised']); }?> <?php echo cs_allow_special_char($cs_theme_option['paypal_currency_sign']);?><?php echo number_format($payment_gross);?></span>
                </div>
                <?php if( isset($cs_node->cause_view) && $cs_node->cause_view == "small" ){?>
                	<?php if(isset($cause_status) && $cause_status <> ''){
								echo '<span class="btn cs-btn-donate cs-bgcolrhvr">'.$cause_status.' </span>';	
