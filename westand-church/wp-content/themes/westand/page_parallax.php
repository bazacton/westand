<?php
    ...
    <?php if(isset($cs_node->parallax_custom_bgcolor) && $cs_node->parallax_custom_bgcolor <> ''){?>
		<style>
            .service-parallax<?php echo cs_allow_special_char($cs_counter_node);?>:before{
                background: none repeat scroll 0 0 <?php echo cs_allow_special_char($cs_node->parallax_custom_bgcolor); ?>;
            
            }
        </style>
    <?php }?>
	<div class="parallaxbg parallax-fullwidth fullwidth bannerarea element_size_100 service-parallax service-parallax<?php echo cs_allow_special_char($cs_counter_node);?>"  style=" <?php if ($cs_node->parallax_custom_img <> '') { ?> background: url(<?php echo cs_allow_special_char($cs_node->parallax_custom_img); ?>) no-repeat center top;<?php } if(isset($cs_node->parallax_height) && !empty($cs_node->parallax_height)){ ?> height: <?php echo cs_allow_special_char($cs_node->parallax_height) ?>px; <?php } ?>  margin-bottom:<?php echo cs_allow_special_char($parallax_margin_bottom)?>px; margin-top:<?php echo cs_allow_special_char($parallax_margin_top); ?>px; padding-top:<?php echo cs_allow_special_char($parallax_padding_top);?>px; padding-bottom:<?php echo cs_allow_special_char($parallax_padding_bottom);?>px; ">
        	<div class="container">
