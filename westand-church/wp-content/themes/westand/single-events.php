...
get_header();
	...
	?>
            ...
            <div class="element_size_100">
                        <div class="event  event-detail <?php echo cs_allow_special_char($post_class);?>">
                            <article>
                                <?php 
								...
								if($inside_event_map == "on" && $address_map <> "0"){
									...
									if($address_map <> "" && $event_loc_lat <> "" && $event_loc_long <>"" && $event_loc_zoom <> ''){ ?>
										...
										jQuery(document).ready(function(){
											event_map("<?php echo addslashes(get_the_title((int)$cs_xmlObject->event_address))  ?>",<?php echo cs_allow_special_char($event_loc_lat); ?>,<?php echo cs_allow_special_char($event_loc_long) ?>,<?php echo cs_allow_special_char($event_loc_zoom) ?>,<?php echo cs_allow_special_char($cs_counter_events); ?>);
										});
										</script>
											<div id="map_canvas<?php echo cs_allow_special_char($cs_counter_events); ?>" class="event-map" style="height:300px; width:100%;"></div>
									<?php }
									echo '</div>';
								}
								?>
                                <div class="detail_inner <?php echo cs_allow_special_char($post_class_no_image);?>">
                                             ...
                                             <?php if($event_start_time <> ""){?>
                                                    <li><i class="fa fa-clock-o"></i>
                                                     <?php if ( $cs_event_meta->event_all_day != "on" ) {?>
                                                                <span class="hd"><?php _e('Event Time','westand'); ?>:</span> <time> <?php echo cs_allow_special_char($event_start_time); if($event_end_time <> ''){ echo "-";  echo cs_allow_special_char($event_end_time); }?></time>
                                                       <?php } else {
                                                           echo '<time>';
                                    	...
                                    	<?php if($cs_event_meta->event_ticket_options <> ""){?> 
                                        	<?php if($cs_event_meta->event_buy_now <> ""){?> 
                                                ...
                                                <?php 
                                                    ...
                                                    ?> 
                                                 <span class="buy-ticket btn cs-bgcolr" <?php if($cs_event_meta->event_ticket_color <> ''){?>style="background-color: <?php echo cs_allow_special_char($cs_event_meta->event_ticket_color);?>" <?php }?>><?php echo cs_allow_special_char($cs_event_meta->event_ticket_options);?></span>
                                                <?php 
                                                }
                                        	?>
                                            <?php } 
											...
											if(isset($cs_xmlObject->event_like) &&  $cs_xmlObject->event_like == "on"){
												...
												?>
												   <a class="btn buy-ticket liks"> <i class="fa fa-thumbs-up"></i> <?php if($cs_theme_option['trans_switcher']== "on"){ _e('Likes','westand');}else{ echo cs_allow_special_char($cs_theme_option['trans_likes']); } ?><span><?php echo cs_allow_special_char($cs_like_counter);?></span></a>
											<?php	
												} else {?>
												  <a  class="likethis btn buy-ticket liks" href="javascript:cs_like_counter('<?php echo cs_allow_special_char($ajax_url); ?>','<?php echo get_template_directory_uri()?>',<?php echo get_the_id()?>)" id="like_this<?php echo get_the_id()?>" > <i class="fa fa-heart-o"></i>  <?php if($cs_theme_option['trans_switcher']== "on"){ _e('Likes','westand');}else{ echo cs_allow_special_char($cs_theme_option['trans_likes']); } ?><span class="count-numbers like_counter<?php echo get_the_id()?>"><?php echo cs_allow_special_char($cs_like_counter); ?></span></a>
													<a class="likes likethis btn buy-ticket liks" id="you_liked<?php echo get_the_id()?>" style="display:none;"><i class="fa fa-heart"></i> <?php if($cs_theme_option['trans_switcher']== "on"){ _e('Likes','westand');}else{ echo cs_allow_special_char($cs_theme_option['trans_likes']); } ?><span class="count-numbers like_counter<?php echo get_the_id()?>"><?php echo cs_allow_special_char($cs_like_counter); ?></span></a>
                            ...
                            <div class="share-post">
                            	<?php if(isset($cs_xmlObject->event_social_sharing) && $cs_xmlObject->event_social_sharing <> ''){cs_addthis_script_init_method();?>
                            				<a href="#" class="btnshare addthis_button_compact"><i class="fa fa-share-square-o"></i><?php if($cs_theme_option['trans_switcher'] == "on"){ _e('Share Now','westand');}else{ echo cs_allow_special_char($cs_theme_option['trans_share_this_post']); } ?> </a>
                                <?php }?>
