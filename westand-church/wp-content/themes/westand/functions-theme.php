		...
		if ( $cs_node->image_element_size <> "" ) { $html .= '<div class="element_size_'.$cs_node->image_element_size.'">'; }
			$html .= '<figure class="lightbox-single image-shortcode" style="float:left; width:'.$cs_node->image_width.'px; height:'.$cs_node->image_height.'px">';
				...
				$html .= '<a class="'.$cs_node->image_style.'" href="'.$href.'" title="'.$cs_node->image_caption.'" '.$data_rel.'>';
                                ...
                                $html .= '<img src="'.$cs_node->image_source.'" style="float:left; width:'.$cs_node->image_width.'px; height:'.$cs_node->image_height.'px" alt="image_url" />';
				if ($cs_node->image_lightbox == "yes"){
				$html .= '</a>';
				}
				...
				$html = '<div class="element_size_'.$cs_node->tabs_element_size.'"><div class="tabs horizontal">';
					...
					foreach( $aaa as $keys=>$vals ){
						...
						if ($tabs_count == 1)
							...
							$tab_active = "";
						$html .= '<div class="tab-pane fade in ' . $tab_active . '" id="tab' . $tab_counter . $tabs_count . '">' . cs_decode_html_tags($vals) . '</div>';
					}
					$html .= '</div></div>';
			$html = $html ;
		}
				...
				$html .= '<div class="panel-group" id="accordion-' . $acc_counter . '">';
					foreach ( $cs_node->accordion as $cs_node ){
						...
						if (!isset($cs_node["title"])){ $cs_node["title"] = '';}
							...
							$html .= '<div class="panel-body"><p>' . cs_decode_html_tags($cs_node->accordion_text) . '</p></div>';
...
if ( ! function_exists( 'cs_services_page' ) ) {
	function cs_services_page() {
    	...
    	?>
        <div class="element_size_<?php echo cs_allow_special_char($cs_node->services_element_size); ?>">
            <!-- Prayer Submit Start -->
            <div class="services border-bottom">
                ...
                foreach ($cs_node->service as $service_info) {
                    ...
                    <?php if ( $service_info->service_type == "type2" ){?>
                    	<article class="<?php echo cs_allow_special_char($service_style);?>">
                        <?php if ($service_info->service_bg_image <> '') { ?>
                            <figure>
                                <img src="<?php echo cs_allow_special_char($service_info->service_bg_image);?>" alt="image_url">
                            </figure>
                            <?php } ?>
                            <div class="text">
                                <h2><a href="<?php echo cs_allow_special_char($service_info->service_link_url); ?>" class="cs-colrhvr"><?php echo cs_allow_special_char($service_info->service_title); ?></a></h2>
                                <?php if ($service_info->service_icon <> '') { ?><i class="fa <?php echo cs_allow_special_char($service_info->service_icon); ?>"></i><?php } ?>
                            </div>
                        </article>
                   	<?php } else {?> 
                        <article class="<?php echo cs_allow_special_char($service_style);?>">
                            <figure><?php if ($service_info->service_icon <> '') { ?><i class="fa <?php echo cs_allow_special_char($service_info->service_icon); ?> fa-2x"></i><?php } ?></figure>
                            <div class="text">
                                <h5><a href="<?php echo cs_allow_special_char($service_info->service_link_url); ?>" ><?php echo cs_allow_special_char($service_info->service_title); ?></a></h5>
           ...
           jQuery("#pattstyles li label") .click(function(){
			   ...
			   var classname=jQuery("#wrappermain-cs") .hasClass("wrapper_boxed"); 
				...
				if(classname == false) { 
					...
					jQuery('body') .css({"background":"url(<?php echo  cs_allow_special_char($theme_path)?>/images/pattern/pattern"+ah+".png)"});
      ...
      jQuery("#backgroundimages li label") .click(function(){
		...
		// var classname=$(".layoutoption li:first-child label") .hasClass("active"); 
			...
			if(classname == false) { 
				...
				$('body') .css({"background":"url(<?php echo  cs_allow_special_char($theme_path)?>/images/background/bg"+ah+".png) no-repeat center center / cover fixed"});
...
var bc ='.cs-bgcolr,.cs-bgcolrhvr:hover,.navigation ul ul li:hover > a,.cycle-pager-active,.pager-desc:before,.fc-header,.fc-event, .pagination ul li a:before, .navigation select,p.stars span a:hover,p.stars span a.active,.comment-form p input[type="submit"]:hover,.widget_newsletter .error,.rev_slider_wrapper, .mercy-style,.wpcf7 form p input[type="submit"]:hover, .password_protected form input[type="submit"],.widget_search input[type="submit"],.tagcloud a:hover,.dropcaptwo:first-letter,.onsale,.add_to_cart_button.button:hover,.woocommerce-pagination ul li span,.woocommerce-pagination ul li a:hover,.woocommerce-message:before,.woocommerce-error:before,.woocommerce-info:before,div.woocommerce .button:hover, .undercunst-box,.flexslider li figcaption h2';
	...
	function reset_color(){
		...
		jQuery("#bgcolor").attr('value',"<?php echo  cs_allow_special_char($cs_theme_option['custom_color_scheme']);?>")
                ...
                <form method="post" id="color_switcher" action="">
                	...
                	<aside class="rowside">
                        ...
                        <div class="switcher-inn">
                            ...
                            <img src="<?php echo cs_allow_special_char($theme_path); ?>/images/admin/img-colorpan.png" alt="image_url">
    
                            <h5><?php _e('Theme Color', 'westand'); ?></h5>
    
                            <input id="bgcolor" name="style_sheet" type="text" class="bg_color" value="<?php echo cs_allow_special_char($_SESSION['wssess_style_sheet']);?>" /></label>
                      ...
                      <div class="innertext">
                        ...
                        <div id="pattstyles" class="itemstyles selectradio">
                            ...
                            <ul>
                                ...
                                <li><label <?php if($_SESSION['wssess_custome_pattern']=="1")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path); ?>/images/pattern/pattern1.png" alt="image_url"><input type="radio" name="custome_pattern" value="1"></label></li>

                                <li><label <?php if($_SESSION['wssess_custome_pattern']=="2")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/pattern/pattern2.png" alt="image_url"><input type="radio" name="custome_pattern" value="2"></label></li>

                                <li><label <?php if($_SESSION['wssess_custome_pattern']=="3")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/pattern/pattern3.png" alt="image_url"><input type="radio" name="custome_pattern" value="3"></label></li>

                                <li><label <?php if($_SESSION['wssess_custome_pattern']=="4")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/pattern/pattern4.png" alt="image_url"><input type="radio" name="custome_pattern" value="4"></label></li>

                                <li><label <?php if($_SESSION['wssess_custome_pattern']=="5")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/pattern/pattern5.png" alt="image_url"><input type="radio" name="custome_pattern" value="5"></label></li>

                                <li><label <?php if($_SESSION['wssess_custome_pattern']=="6")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/pattern/pattern6.png" alt="image_url"><input type="radio" name="custome_pattern" value="6"></label></li>

                                <li><label <?php if($_SESSION['wssess_custome_pattern']=="7")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/pattern/pattern7.png" alt="image_url"><input type="radio" name="custome_pattern" value="7"></label></li>

                                <li><label <?php if($_SESSION['wssess_custome_pattern']=="8")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/pattern/pattern8.png" alt="image_url"><input type="radio" name="custome_pattern" value="8"></label></li>

                                <li><label <?php if($_SESSION['wssess_custome_pattern']=="9")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/pattern/pattern9.png" alt="image_url"><input type="radio" name="custome_pattern" value="9"></label></li>

                                <li><label <?php if($_SESSION['wssess_custome_pattern']=="10")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/pattern/pattern10.png" alt="image_url"><input type="radio" name="custome_pattern" value="10"></label></li>
                                 <li><label <?php if($_SESSION['wssess_custome_pattern']=="11")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/pattern/pattern11.png" alt="image_url"><input type="radio" name="custome_pattern" value="11"></label></li>
                                  <li><label <?php if($_SESSION['wssess_custome_pattern']=="12")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/pattern/pattern12.png" alt="image_url"><input type="radio" name="custome_pattern" value="12"></label></li>
                                    <li><label <?php if($_SESSION['wssess_custome_pattern']=="13")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/pattern/pattern13.png" alt="image_url"><input type="radio" name="custome_pattern" value="13"></label></li>
                                      <li><label <?php if($_SESSION['wssess_custome_pattern']=="14")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/pattern/pattern14.png" alt="image_url"><input type="radio" name="custome_pattern" value="14"></label></li>
                                        <li><label <?php if($_SESSION['wssess_custome_pattern']=="15")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/pattern/pattern15.png" alt="image_url"><input type="radio" name="custome_pattern" value="15"></label></li>
                    ...
                    <aside class="rowside">
                        ...
                        <div id="backgroundimages" class="selectradio">
                            ...
                            <ul>
                            	...
                            	<li><label <?php if($_SESSION['wssess_bg_img']=="1")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/background/background1.png" alt="background1"><input type="radio" name="bg_img" value="1"></label></li>

                                <li><label <?php if($_SESSION['wssess_bg_img']=="2")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/background/background2.png" alt="background1"><input type="radio" name="bg_img" value="2"></label></li>

                                <li><label <?php if($_SESSION['wssess_bg_img']=="3")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/background/background3.png" alt="background1"><input type="radio" name="bg_img" value="3"></label></li>

                                <li><label <?php if($_SESSION['wssess_bg_img']=="4")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/background/background4.png" alt="background1"><input type="radio" name="bg_img" value="4"></label></li>

                                <li><label <?php if($_SESSION['wssess_bg_img']=="5")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/background/background5.png" alt="background1"><input type="radio" name="bg_img" value="5"></label></li>

                                <li><label <?php if($_SESSION['wssess_bg_img']=="6")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/background/background6.png" alt="background1"><input type="radio" name="bg_img" value="6"></label></li>

                                <li><label <?php if($_SESSION['wssess_bg_img']=="7")echo "class='active'";?> ><img src="<?php echo cs_allow_special_char($theme_path);?>/images/background/background7.png" alt="background1"><input type="radio" name="bg_img" value="7"></label></li>
...
.cs-colr, .cs-colrhvr:hover,.followus a:hover,.cs-timeline article:hover .text a,.widget_categories ul li:hover a, .tagclouds a:hover,.post-options li a:hover,.cs-timeline article:hover h2,.team-shortcode article:hover .text h3, .pagination ul li:hover a,div.woocommerce a:hover,p a:hover, .archive-page article p strong,#footer p a,.cs-carousel-control .arrow-left:hover .fa-angle-left,.cs-carousel-control .arrow-right:hover .fa-angle-right {
	color:<?php echo  cs_allow_special_char($cs_color_scheme); ?> !important;
}
...
.cs-bgcolr,.cs-bgcolrhvr:hover,.navigation ul ul li:hover > a,.cycle-pager-active,.pager-desc:before,.fc-header,.fc-event, .pagination ul li a.active:after ,.pagination ul li a:before, .navigation select,p.stars span a:hover,p.stars span a.active,.comment-form p input[type="submit"]:hover,.widget_newsletter .error,.rev_slider_wrapper, .mercy-style,.wpcf7 form p input[type="submit"]:hover, .password_protected form input[type="submit"],.widget_search input[type="submit"],.tagcloud a:hover,.cart-sec span,.dropcaptwo:first-letter,.onsale,.add_to_cart_button.button:hover,.woocommerce-pagination ul li span,.woocommerce-pagination ul li a:hover,.woocommerce-message:before,.woocommerce-error:before,.woocommerce-info:before,div.woocommerce .button:hover, .undercunst-box,.flexslider li figcaption h2, btn{
	background-color:<?php echo cs_allow_special_char($cs_color_scheme); ?> !important;
}
.cs-bdrcolr,.navigation > ul > li > a:before,.woocommerce a.button, .woocommerce button.button, .woocommerce input.button, .woocommerce #respond input#submit,.woocommerce #content input.button,.woocommerce-page a.button,.woocommerce-page button.button,.woocommerce-page input.button,.woocommerce-page #respond input#submit,.woocommerce-page #content input.button,.woocommerce-info,.woocommerce-message, .woocommerce-error{
	border-color:<?php echo cs_allow_special_char($cs_color_scheme); ?> !important;
}
.widget_newsletter .error:before{
	border-color: transparent <?php echo cs_allow_special_char($cs_color_scheme); ?> !important;
}
#header{ 
	background-color: <?php echo cs_allow_special_char($menu_bg_color); ?> !important;
	
}
.navigation > ul > li > a{
	 color:  <?php echo cs_allow_special_char($menu_font_color); ?> !important;
}
</style>
...
if ( ! function_exists( 'cs_get_header' ) ) { 
	...
	?>
        ...
        <div class="container">
            ...
            <a href="#" class="btn cs-bgcolr" data-toggle="modal" data-target="#myModal"><?php if($cs_theme_option['trans_switcher'] == "on"){ _e('Support us','westand');}else{ echo  cs_allow_special_char($cs_theme_option['donation_btn_title']); } ?></a>
            <?php }?>
				...
				echo '<div id="sliderbanner">';
					echo '<div class="cycle-slideshow" 
							...
							while ($custom_query->have_posts()) : $custom_query->the_post();
								...
								?>
									<article class="<?php echo  cs_allow_special_char($post->ID); ?>">
                                        <?php if($image_url_full <> ''){?><img src="<?php echo  cs_allow_special_char($image_url_full);?>" alt="image_url_full"><?php }?>
                                               <div class="caption">
                                                	<h2><?php the_title();?></h2>
                                               		<p><?php  cs_get_the_excerpt('113',false);?></p>
                                                	<a href="<?php the_permalink();?>" class="btn cs-bgcolrhvr"><?php if($cs_theme_option['trans_switcher'] == "on"){ _e('Continue Reading','westand');}else{ echo  cs_allow_special_char($cs_theme_option['trans_read_more']); }?></a>
                                               </div> 
                                    </article>
                                <?php
								}
...
if ( ! function_exists( 'cs_flex_slider' ) ) {
	...
	function cs_flex_slider($width,$height,$slider_id){
		...
		<div id="flexslider<?php echo  cs_allow_special_char($cs_counter_node); ?>">
					...
					foreach ( $cs_xmlObject_flex->children() as $as_node ){
                        ...
                        <li>
                            ...
                            <figure>
                                ...
                                <img src="<?php echo  cs_allow_special_char($image_url); ?>" alt="image_url_full">   
                                     	...
                                     	<h2 class="colr">
											<?php 
												if($as_node->link <> ''){ 
													...
													echo  cs_allow_special_char($as_node->title);
		...
		<script type="text/javascript">
			...
			jQuery(document).ready(function(){
				...
				var speed = <?php echo  cs_allow_special_char($cs_theme_option['flex_animation_speed']); ?>; 

				var slidespeed = <?php echo  cs_allow_special_char($cs_theme_option['flex_pause_time']); ?>;

				jQuery('#flexslider<?php echo cs_allow_special_char($cs_counter_node); ?> .flexslider').flexslider({

					animation: "<?php echo cs_allow_special_char($cs_theme_option['flex_effect']); ?>", // fade
					slideshow: <?php echo  cs_allow_special_char($auto_play);?>,
...
if ( ! function_exists( 'cs_social_network' ) ) {
	...
	function cs_social_network($icon_type='',$tooltip = ''){
		...
		if ( isset($cs_theme_option['social_net_url']) and count($cs_theme_option['social_net_url']) > 0 ) {
					...
					<?php if($val != ''){?><a title="social_net_tooltip" href="<?php echo  cs_allow_special_char($val);?>" data-original-title="<?php echo cs_allow_special_char( $cs_theme_option['social_net_tooltip'][$i]);?>" data-placement="top" <?php echo cs_allow_special_char($tooltip_data);?> class="colrhover"  target="_blank"><?php if($cs_theme_option['social_net_awesome'][$i] <> '' && isset( $cs_theme_option['social_net_awesome'][$i])){?> 

                    <span class="fa-stack fa-lg">

                        <em class="fa fa-square fa-stack-2x"></em>

                        <em class="<?php echo cs_allow_special_char($cs_theme_option['social_net_awesome'][$i]);?> <?php echo cs_allow_special_char($icon); ;?> fa-stack-1x fa-inverse fa"></em>

                    </span>

					

					<?php } else {?><img src="<?php echo  cs_allow_special_char($cs_theme_option['social_net_icon_path'][$i]);?>" alt="<?php echo cs_allow_special_char($cs_theme_option['social_net_tooltip'][$i]);?>" /><?php }?></a><?php }
...
if ( ! function_exists( 'cs_social_network_widget' ) ) {
	...
	function cs_social_network_widget($icon_type='',$tooltip = ''){
		...
		if ( isset($cs_theme_option['social_net_url']) and count($cs_theme_option['social_net_url']) > 0 ) {
					...
					<?php if($val != ''){?><a class="cs-colrhvr" title="social_net_tooltip" href="<?php echo cs_allow_special_char($val);?>" data-original-title="<?php echo cs_allow_special_char($cs_theme_option['social_net_tooltip'][$i]);?>" data-placement="top" <?php echo cs_allow_special_char($tooltip_data);?> target="_blank"><?php if($cs_theme_option['social_net_awesome'][$i] <> '' && isset($cs_theme_option['social_net_awesome'][$i])){?> 


                        <i class="fa <?php echo cs_allow_special_char($cs_theme_option['social_net_awesome'][$i]);?>"></i>


					

					<?php } else {?><img src="<?php echo cs_allow_special_char($cs_theme_option['social_net_icon_path'][$i]);?>" alt="<?php echo cs_allow_special_char($cs_theme_option['social_net_tooltip'][$i]);?>" /><?php }?>
...
function cs_wrapper_class(){
	...
	if ( isset($_POST['layout_option']) ) {
		...
		echo cs_allow_special_char($_SESSION['wssess_layout_option'] = $_POST['layout_option']);

	}

	elseif ( isset($_SESSION['wssess_layout_option']) and !empty($_SESSION['wssess_layout_option'])){

		echo cs_allow_special_char($_SESSION['wssess_layout_option']);

	}

	else {

		echo cs_allow_special_char($cs_theme_option['layout_option']);
...
add_action('widgets_init', 'westand_custome_siderbar');
function westand_custome_siderbar() {
...
if ( ! function_exists( 'cs_under_construction' ) ) {
	...
	function cs_under_construction(){ 
		...
		<div id="midarea">
			<?php echo '<p>'.wp_specialchars_decode($cs_theme_option['under_construction_text']).'</p>';
			...
			<script type="text/javascript" src="<?php echo get_template_directory_uri(); ?>/scripts/frontend/jquery.countdown.js"></script>
			 <script type="text/javascript">
				...
				jQuery(function () {
					...
					austDay = new Date(<?php echo cs_allow_special_char($year); ?>,<?php echo cs_allow_special_char($month); ?>-1,<?php echo cs_allow_special_char($day) 	; ?>);
