...
function cache_json($id, $max_tweets, $time) {
   ...
   // $cache_folder = get_template_directory() . '/include/twitteroauth/cache/'; //Twitter cache directory
    $cache = cs_framework::plugin_dir() . 'twitteroauth/cache/' . $id . '.json'; //Twitter cache directory
    $cache_folder = cs_framework::plugin_dir() . 'twitteroauth/cache/'; //Twitter cache directory
...
function display_tweets($id, $style = '', $max_tweets = 10, $max_cache_tweets = 10, $time = 60) {
    ...
    if (!empty($tweets)) {
        ...
        foreach ($tweets as $tweet) {
            ...
            $twitter .= "<div class='swiper-slide'><div class='tweet'><h5><i class='fa fa-twitter'></i></h5><h4>" . $tweet . "</h4><p><a href='https://twitter.com/" . $id . "'>@" . $id . "</a></p>";

            if (isset($style)) {
                if (!empty($style)) {
                    $when = ($style == 'time_since' ? '' : '');
                    $twitter.="<ul class='tweet-panel'><li>" . $when . "";
                    ...
                    $twitter .= "</li></ul></div></div>"; //end of List
                }
            }
    ...
    echo "<div class='swiper-container swiper-container" . $rand_id . "'><div class='swiper-wrapper'>" . $twitter . "</div></div>";
}
