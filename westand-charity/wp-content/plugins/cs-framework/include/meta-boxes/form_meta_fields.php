<?php
	...
	public function save_page_option($post_id='') {
		...
		# Event Save 
		if ( isset($_POST['cs_event_num_repeat'] ) && $_POST['cs_repeat_access'] == '' ) {
			...
			$to_date   = $_POST["cs_event_to_date"];
				...
				for ( $i = 1; $i < $_POST['cs_event_num_repeat']; $i++ ) {
					...
					if ( $_POST['cs_event_repeat'] <> 0 ) {
						...
						update_post_meta( $inserted_id, 'cs_event_from_date', $event_from_date );
						update_post_meta( $inserted_id, 'cs_event_to_date', $event_to_date );
                ...
                $cs_output .= '<div class=\'radio-image-wrapper\'>';
                  ...
                  $cs_output .= '<label for="page_radio_1">';
				 	  $cs_output .= '<span class="ss">';
				 	 	$cs_output .= '<img src="'.get_template_directory_uri().'/include/assets/images/no_sidebar.png"  alt="" />';
					  $cs_output .= '</span>';
						$cs_output .= '<span '.$cs_none_checklist.' id="check-list"></span>';
					$cs_output .= '</label>';
                  $cs_output .= '<span class="title-theme">'.__("Full Width","Hotel").'</span></div>';
                $cs_output .= '<div class=\'radio-image-wrapper\'>';
                  ...
                  $cs_output .= '<label for="page_radio_2">';
				     $cs_output .= '<span class="ss">';
				 	 	$cs_output .= '<img src="'.get_template_directory_uri().'/include/assets/images/sidebar_right.png" alt="" />';
				  	 $cs_output .= '</span>';
						$cs_output .= '<span '.$cs_right_checklist.' id="check-list"></span>';
				  $cs_output .= '</label>';
                  $cs_output .= '<span class="title-theme">'.__("Sidebar Right","Hotel").'</span> </div>';
                $cs_output .= '<div class=\'radio-image-wrapper\'>';
                  ...
                  $cs_output .= '<label for="page_radio_3">';
				  	$cs_output .= '<span class="ss">';
				  		$cs_output .= '<img src="'.get_template_directory_uri().'/include/assets/images/sidebar_left.png" alt="" />';
					$cs_output .= '</span>';
					$cs_output .= '<span '.$cs_left_checklist.' id="check-list"></span>';
					$cs_output .= '</label>';
                  $cs_output .= '<span class="title-theme">'.__("Sidebar Left","Hotel").'</span> </div>';
              $cs_output .= '</div>';
            $cs_output .= '</div>';
          $cs_output .= '</li>';
        $cs_output .= '</ul>';
	...
	public function cs_form_fileupload_render( $params = '' ) {
		...
		$cs_output  = '<ul class="form-elements">';
			...
			$cs_output .= '<li class="to-field">';
				...
				$cs_output .= '<label class="browse-icon"><input name="cs_' .sanitize_html_class( $id.$cs_random_id ). '"  type="button" class="uploadMedia left" value="'.__('Browse','Hotel').'"/></label>';
			$cs_output .= '</li>';
		$cs_output .= '</ul>';
	...
	public function cs_get_icon_for_attachment( $post_id ) {
	  ...
	  switch ( $type ) {
		...
		case 'video/quicktime':
		  return '<img src="'.get_template_directory_uri().'/include/assets/images/video.png'.'" alt=""/>'; break;
		case 'text/csv':
		case 'text/plain': 
		case 'text/xml':
		 return '<img src="'.get_template_directory_uri().'/include/assets/images/attachment.png'.'" alt=""/>'; break;
		default:
		 return '<img src="'.get_template_directory_uri().'/include/assets/images/attachment.png'.'" alt=""/>'; break;
	  }
	}
