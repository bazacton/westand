 <?php
	...
	?>
    <div class="element_size_<?php echo cs_allow_special_char($cs_node->blog_element_size); ?>">
    <?php	
		$blog_view = $cs_node->cs_blog_view;
		 ...
		 ?>
	<div class="cs-blog <?php  echo cs_allow_special_char($blog_view); ?>">
					...
					?>
                        ...
                        <?php 
							if($image_url <> ""){
								...
								<a href="'.get_permalink().'" class="cs-colrhvr" ><img src="'.$image_url.'" alt="image_url" ></a>
								<figcaption>
                    	 ...
                    	 <?php
  								echo '<figure>';
  							 	if ( $post_view == "Slider"  and $post_slider <> ''){
                                	...
                                	if($image_url <> ''){ 
									echo '<a href="'.get_permalink().'" ><img src="'.$image_url.'" alt="image_url" ></a>';
									 }
				...
				$request_server_names = cs_glob_server("SERVER_NAME");
									...
									if($url['host'] == $request_server_names){
 									$poster_url = '';
										if($post_featured_image=='on'){$poster_url = $image_url;}
										 if($image_url <> ''){ echo "<a href='".get_permalink()."'><img src=".$image_url." alt='image_url' ></a>";}
                                    ?>
                                        <figcaption class="gallery">
                                                      <a data-toggle="modal" data-target="#myModal<?php echo cs_allow_special_char($post->ID);?>"  onclick="cs_video_load('<?php echo get_template_directory_uri();?>', <?php echo cs_allow_special_char($post->ID);?>, '<?php echo cs_allow_special_char($post_video);?>','<?php echo cs_allow_special_char($poster_url);?>');" href="#"><i class="fa fa-video-camera fa-2x"></i></a>
                                         </figcaption>
									<?php
									}else{
                        ...
                        <!-- Blog Post Thumbnail End -->
                                <div class="calendardate-wrap">
									...
									?>
									<a class="btn cs-bgcolr"><i class="fa fa-heart-o"></i><?php echo cs_allow_special_char($cs_like_counter);?></a></li>
                                ...
                                <div class="text">
                                    ...
                                    <div class="cs-blog-panel">
                                        ...
                                        <a class="btnshare addthis_button_compact"><i class="fa fa-share-square-o"></i><?php if($cs_theme_option['trans_switcher'] == "on"){ _e('Sahre','westand');}else{ echo cs_allow_special_char($cs_theme_option['trans_share_this_post']); } ?></a>
                                    </div>
                                </div>
                     </article>
                     <?php if($post_view == "Video"){?>
                    <div class="modal fade" id="myModal<?php echo cs_allow_special_char($post->ID);?>" tabindex="-1" role="dialog" aria-hidden="true"></div>
                    <?php }?>
                    	...
                    	<div class="swiper-container">
                        	<div class="swiper-wrapper">
                                    	...
                                    	<?php 
                                            ...
                                            <article <?php post_class($no_image);?>>
                                                <figure>
                                                    <?php if($image_url <> ""){?><img src="<?php echo cs_allow_special_char($image_url);?>" alt="image_url"><?php }?>
                                                    <figcaption>
