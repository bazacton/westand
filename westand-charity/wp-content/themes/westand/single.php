                                    ...
                                    if( $post_view == "Slider" and $post_slider <> ''){
                                        echo '<figure class="detail_figure">';
										 ...
										 echo '<img src="'.$image_url.'" >';
										  ...
										  $request_server_names = '';
			if(function_exists('cs_glob_server')) {
				$request_server_names = cs_glob_server("SERVER_NAME");
			}
                                         if($url['host'] == $request_server_names){?>
                                            <figure class="detail_figure">
                                            <video width="<?php echo cs_allow_special_char($width);?>" class="mejs-wmp" height="100%"  style="width: 100%; height: 100%;" src="<?php echo cs_allow_special_char($post_video); ?>"  id="player1" poster="<?php if($post_featured_image == "on"){ echo cs_allow_special_char($image_url); } ?>" controls="controls" preload="none"></video>
                                            </figure>
                                        <?php
                                        }else{
                                         ...
                                         <figcaption class="gallery">
                                            <div class="audiowrapp fullwidth">
                                                <audio style="width:100%;" src="<?php echo cs_allow_special_char($post_audio); ?>" type="audio/mp3" controls="controls"></audio>
                                            </div>  
                                        </figcaption>
                                        <?php
                                       echo '</figure>';
                             ...
                             <div class="cs-blog-panel">
                                <ul class="post-options">
                                    <li><i class="fa fa-calendar"></i><time><?php echo cs_allow_special_char(get_the_date()); ?></time></li>
                                    <li><i class="fa fa-user"></i><a href="<?php echo cs_allow_special_char(get_author_posts_url(get_the_author_meta('ID'))); ?>"><?php echo cs_allow_special_char(get_the_author()); ?></a></li>
									<?php 
									...
									?>
									   <li><a><i class="fa fa-heart"></i><?php echo cs_allow_special_char($cs_like_counter);?></a></li>
								<?php	
									} else {?>
									  <li><a  class="likethis" href="javascript:cs_like_counter('<?php echo cs_allow_special_char($ajax_url); ?>','<?php echo get_template_directory_uri()?>',<?php echo get_the_id()?>)" id="like_this<?php echo cs_allow_special_char(get_the_id()); ?>" ><i class="fa fa-heart-o"></i> <span class="count-numbers like_counter<?php echo get_the_id()?>"><?php echo cs_allow_special_char($cs_like_counter); ?></span> </a>
										<a class="likes likethis" id="you_liked<?php echo get_the_id()?>" style="display:none;"><i class="fa fa-heart"></i><span class="count-numbers like_counter<?php echo get_the_id()?>"><?php echo cs_allow_special_char($cs_like_counter); ?></span> </a>
										<div id="loading_div<?php echo cs_allow_special_char(get_the_id()); ?>" style="display:none;"><i class="fa fa-spinner fa-spin"></i></div>
                                 </li>
								<?php }?>
                            ...
                            <div class="share-post">
								...
								?>
                                <a class="addthis_button_compact btnshare" href="#"><i class="fa fa-share-square-o"></i><?php if($cs_theme_option['trans_switcher'] == "on"){ _e('Sahre','westand');}else{ echo cs_allow_special_char($cs_theme_option['trans_share_this_post']); } ?> </a>
                                <?php
								}
								?>
